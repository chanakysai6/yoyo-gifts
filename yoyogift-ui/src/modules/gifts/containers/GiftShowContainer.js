...
class GiftShowContainer extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.login) {
      this.props
        ...
        .then(() => {
          this.setState({
            balance_points: this.props.login.balance_points,
  ...
  validateSend = async (sendTo) => {
    if (
      ...
      const currentBalance = this.props.login.balance_points
  ...
  sendMail = (email, name) => {
    ...
    var service_id = 'service_vb6p9cs'
    // Template you will use
    var template_id = 'template_75vvgzo'
    // Send message using your client
  
    emailjs
      .send(service_id, template_id, payload, 'PgrCZLWeG48-Dtf0L')
      .then(
        function (response) {
...
const mapStateToProps = (state, ownProps) => {
  return {
    ...
    login: state.login.detailsObject.data,
